...
Версия `0.32.0` добавляет schema-v3 fencing counters. Каждый write-resource
получает persistent monotonic token; release, expiry и recovery не сбрасывают
историю. Renew сохраняет исходный token, а read lease token не получает.
...
Версия `0.36.0` добавляет второй независимый case `python-project-audit`.
Он проверяет read-only parallel evidence gathering по source, README и
pyproject, capability acquisition и integration child evidence.

`BenchmarkReport` получил явную benchmark identity, а CLI выбирает case и
...
Версия `0.39.0` добавляет external `TaskRetryPolicy`: allowlist codes, bounded
attempts и точный idempotency key по node id. Executor может только запросить
retry; policy mismatch или отсутствие policy завершаются terminal failure.

Benchmark retryable side effect подтверждает один transient failure, две
попытки с одинаковым key и ровно один materialized commit. После трёх runs
четырёх-case profile даёт роли `parallel` 4 из 4 побед. Independent work
перекрывается, но retried commit остаётся dependent leaf.

Task graph persistence переведена на schema v2 с backward-readable schema v1.
...
Версия `0.41.0` добавляет persistent elapsed retry deadline и deterministic
jitter. Retry window начинается при первом авторизованном повторе и включает
ожидание, последующие executions, checkpoint и resume. Epoch clock внедряется
явно; ошибка, regression времени и исчерпание окна завершаются fail-closed.

Jitter вычисляется через SHA-256 от immutable seed, graph id, node id и номера
retry. Разные scheduler identities получают разные задержки без
process-global randomness, а одинаковый вход полностью воспроизводим.

Task graph persistence переведена на schema v4 с чтением v1-v4.

Версия `0.42.0` добавляет secret-safe aggregate retry telemetry. Из event log
извлекаются только counts, retry codes, rejection reasons и суммы delay/jitter.
Idempotency keys и произвольные payload fields в telemetry report не попадают.

Новый benchmark запускает два spawned scheduler process с общим persistent
lease registry. Второй scheduler начинает acquisition после подтверждения,
что первый уже удерживает write lease. Пять последовательных targeted runs
подтвердили bounded contention retry, отсутствие overlap критических секций,
различный deterministic jitter и terminal cleanup обоих процессов.
