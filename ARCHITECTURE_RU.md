...
### Retry Telemetry и Multi-Process Contention Benchmark

В `0.42.0` добавлен `aggregate_retry_telemetry`. Он читает публичные task
events, но переносит в aggregate report только allowlisted данные:

- количество scheduled/rejected retry;
- количество начатых, завершённых и отменённых waits;
- суммы delay и deterministic jitter;
- counts по retry codes и rejection reasons.

Idempotency keys, owner ids и произвольные event payload fields не копируются.
Telemetry является наблюдением и не влияет на scheduler decisions.

Benchmark `multiprocess-lease-contention` запускает через `spawn` два реальных
scheduler process с общим file lease registry. Scheduler B начинает acquisition
после сигнала из executor A, то есть эксперимент измеряет именно contention
уже удерживаемого write lease, а не случайную гонку registry lock.

Acceptance требует:

- оба scheduler завершаются;
- write execution intervals не пересекаются;
- зафиксирован хотя бы один реальный contention retry;
- разные immutable seeds дают разные deterministic jitter;
- worker processes завершены или принудительно сняты bounded cleanup;
- aggregate telemetry не содержит idempotency keys.
...
1. Включить retry telemetry в versioned service/run reports.
2. Добавить adaptive retry policy как внешний advisor, который предлагает
   policy следующего run, но не меняет текущий scheduler decision.
